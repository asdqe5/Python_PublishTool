...
def initUIFunc(self):
    ...
    UI 초기 세팅
    '''
    self.ui.team_user_lineEdit.setText(getpass.getuser())
    ...
    self.ui.team_project_comboBox.clear()
    ...
    self.ui.team_project_comboBox.addItems([""] + newProjectList)

    self.ui.team_sequence_comboBox.clear()
    self.ui.team_shot_comboBox.clear()
    self.ui.team_task_comboBox.clear()
    self.ui.team_ext_comboBox.clear()
    self.ui.team_shotgunmov_checkBox.setChecked(False)
    self.ui.team_version_comboBox.clear()
    self.ui.team_note_textEdit.clear()
    self.ui.team_final_radioButton.setChecked(True)
    self.ui.team_publog_textEdit.clear()
    self.ui.team_publog_textEdit.setReadOnly(True)
    self.ui.team_ccGroup_lineEdit.clear()
    self.ui.team_ccGroup_lineEdit.setReadOnly(True)
    self.ui.team_ccPerson_lineEdit.clear()
    self.ui.team_ccPerson_lineEdit.setReadOnly(True)

    # 리눅스는 한글 입력을 못하니까 안내문을 띄워준다.
    if sys.platform == "linux2":
        self.ui.team_note_textEdit.setToolTip(u"<html><head/><body><p>한글 입력이 안됩니다.<br>메모장을 이용해주세요.</p></body></html>")
    elif sys.platform == "win32":
        self.ui.team_note_toolButton.setVisible(False)

    # Publish Data 테이블 설정
    while (self.ui.team_publish_tableWidget.rowCount() > 0):
        self.ui.team_publish_tableWidget.removeRow(0)
    self.ui.team_publish_tableWidget.setColumnCount(len(self.team_column_headers))
    self.ui.team_publish_tableWidget.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
    self.ui.team_publish_tableWidget.setHorizontalHeaderLabels(self.team_column_headers)


    # Publish File에 따라서 테이블 세팅 설정
    if self.ui.team_pubFile_comboBox.currentText() == "Only Nuke":
        self.ui.team_ext_comboBox.setEnabled(False)
        self.ui.team_shotgunmov_checkBox.setEnabled(False)
    else:
        self.ui.team_ext_comboBox.setEnabled(True)
        self.ui.team_shotgunmov_checkBox.setEnabled(True)
...
def setSeqFunc(self):
    ...
    if self.ui.team_project_comboBox.currentText() == "":
        return
    projectName = self.ui.team_project_comboBox.currentText()
    ...
    self.ui.team_sequence_comboBox.addItems([""] + seqList)
...
def setShotFunc(self):
    ...
    if self.ui.team_sequence_comboBox.currentText() == "":
        return
    seqName = self.ui.team_sequence_comboBox.currentText()
    ...
    self.ui.team_shot_comboBox.addItems([""] + shotList)
...
def setTaskFunc(self):
    ...
    if self.ui.team_shot_comboBox.currentText() == "":
        return
    shotName = self.ui.team_shot_comboBox.currentText()
    teamName = "comp"
    ...
    self.ui.team_task_comboBox.addItems([""] + taskList["devl"])

def setCheckFunc(self):
    '''
    Publish File ComboBox를
    체크하는 함수
    '''
    if self.ui.team_pubFile_comboBox.currentText() == "Only Nuke":
        setNukeVerFunc(self)
...
def setExtFunc(self):
    ...
    if self.ui.team_task_comboBox.currentText() == "":
        return
    
    taskName = self.ui.team_task_comboBox.currentText()
    ...
    if os.path.exists(path):
        ...
        self.ui.team_ext_comboBox.addItems([""] + extList)
    else:
        dial = QtWidgets.QMessageBox()
        dial.setWindowTitle("Error")
...
def setNukeVerFunc(self):
    ...
    path = os.path.join(SHOW_PATH, projectName, "shot", seqName, shotName, teamName, "devl", taskName, "nuke")
    if os.path.exists(path):
        if not os.listdir(path):
            dial = QtWidgets.QMessageBox()
            dial.setWindowTitle("Error")
        ...
        for nv in nukeVer:
            ...
            # 폴더인지 확인
            if os.path.isdir(os.path.join(path, nv)):
                continue
            ...
            nukeVerList.append(nv)

        self.ui.team_version_comboBox.addItems([""] + nukeVerList)
...
def setVerFunc(self):
    ...
    teamName = "comp"
    taskName = self.ui.team_task_comboBox.currentText()
    if self.ui.team_shotgunmov_checkBox.isChecked():
        self.ui.team_confirm_radioButton.setChecked(True)
    else:
        self.ui.team_final_radioButton.setChecked(True)

    if self.ui.team_ext_comboBox.currentText() == "":
        return
    
    extName = self.ui.team_ext_comboBox.currentText()
    ...
    if os.path.exists(path):
        ...
        if extName.startswith("mov"):
            if self.ui.team_shotgunmov_checkBox.isChecked():
                ...
                self.ui.team_version_comboBox.addItems([""] + movverList)
            ...
            if not self.ui.team_shotgunmov_checkBox.isChecked():
                ver = os.listdir(path)
                ver.sort()
                ...
                self.ui.team_version_comboBox.addItems([""] + verList)
            else:
                self.ui.team_shotgunmov_checkBox.setChecked(False)

    else:
        if extName == "mov" and self.ui.team_shotgunmov_checkBox.isChecked(): # mov 폴더는 존재하지 않고 샷건용 mov만 존재하는 경우
            dpath = os.path.dirname(path)
            movver = os.listdir(dpath)
            movver.sort()
...
def setCCGroupFunc(self):
    ...
    '''
    # 버튼을 비활성화한다.
    self.ui.team_ccGroup_pushButton.setEnabled(False)
    self.ui.team_ccPerson_pushButton.setEnabled(False)
...
def addCCGroupWidget(self, ccList, widget):
    ...
    widget.close()
    self.ui.team_ccGroup_lineEdit.setText(ccText)
    self.ui.team_ccGroup_pushButton.setEnabled(True)
    self.ui.team_ccPerson_pushButton.setEnabled(True)
...
def addCCPersonWidget(self, ccList, widget):
    ...
    widget.close()
    self.ui.team_ccPerson_lineEdit.setText(ccText)
    self.ui.team_ccGroup_pushButton.setEnabled(True)
    self.ui.team_ccPerson_pushButton.setEnabled(True)
...
def setPublishTableFunc(self):
    ...
    if not self.ui.team_pubFile_comboBox.currentText() == "Only Nuke":
        if self.ui.team_ext_comboBox.currentText() == "":
            dial = QtWidgets.QMessageBox()
            dial.setWindowTitle("Error")
    ...
    if self.ui.team_version_comboBox.currentText() == "":
        dial = QtWidgets.QMessageBox()
        dial.setWindowTitle("Error")
    ...
    taskName = self.ui.team_task_comboBox.currentText()
    extName = self.ui.team_ext_comboBox.currentText()
    verName = self.ui.team_version_comboBox.currentText()
    ccGroup = self.ui.team_ccGroup_lineEdit.text()
    ccPerson = self.ui.team_ccPerson_lineEdit.text()
    ...
    if self.ui.team_shotgunmov_checkBox.isChecked():
        devlRenderPath = os.path.join(devlPath, "render", verName) 
        pubRenderPath = os.path.join(pubPath, version, "render", verName)

    else: # 프로젝트별 예외처리
        devlRenderPath = os.path.join(devlPath, "render", extName, verName)
        pubRenderPath = os.path.join(pubPath, version, "render", extName, verName)
    ...
    if os.path.exists(nukePath):
        ...
        if nukeFileName in nukeFiles:
            ...
            if not projectName in setting:
                ...
                # 납품용으로 PUB을 하는 경우
                if self.ui.team_final_radioButton.isChecked():
    ...
    devlNukePath = os.path.join(nukePath, nukeFileName)
    pubNukePath = os.path.join(pubPath, version, "nuke", nukeFileName)
    tableRow = self.ui.team_publish_tableWidget.rowCount()
    tableColumn = self.ui.team_publish_tableWidget.columnCount()
    for r in range(tableRow):
        # nuke 파일만 확인하는 경우
        if self.ui.team_pubFile_comboBox.currentText() == "Only Nuke":
            if devlNukePath == self.ui.team_publish_tableWidget.item(r, self.team_column_idx_lookup["DevlNukePath"]).text():
                dial = QtWidgets.QMessageBox()
                dial.setWindowTitle("Error")
        ...
        # render 파일로 확인하는 경우
            ...
            if devlRenderPath == self.ui.team_publish_tableWidget.item(r, self.team_column_idx_lookup["DevlRenderPath"]).text():
                dial = QtWidgets.QMessageBox()
                dial.setWindowTitle("Error")
                dial.setText("테이블에 이미 같은 파일이 존재합니다.")
                ok_btn = dial.addButton("OK", QtWidgets.QMessageBox.YesRole)
    ...
    note = self.ui.team_note_textEdit.toPlainText()

    # nuke 파일만 publish하는 경우 render 경로는 필요없다
    if self.ui.team_pubFile_comboBox.currentText() == "Only Nuke":
        extName = "nk"
        devlRenderPath = ""
        pubRenderPath = ""
    
    # render 파일만 publish 하는 경우 nuke 경로는 필요없다
    elif self.ui.team_pubFile_comboBox.currentText() == "Only Render":
        devlNukePath = ""
        pubNukePath = ""
    
    tableData = [projectName, seqName, shotName, taskName, verName, extName, pubRenderPath, pubNukePath, devlRenderPath, devlNukePath, frame_range, duration, resolution, note, ccGroup, ccPerson]
    self.ui.team_publish_tableWidget.insertRow(tableRow)
    for j in range(tableColumn):
        item = QtWidgets.QTableWidgetItem(tableData[j])
        item.setTextAlignment(QtCore.Qt.AlignVCenter | QtCore.Qt.AlignHCenter)
        self.ui.team_publish_tableWidget.setItem(tableRow, j, item)

    self.ui.team_publish_tableWidget.resizeColumnsToContents()
    self.ui.team_publish_tableWidget.resizeRowsToContents()
...
def minusPublishTableFunc(self):
    ...
    '''
    if self.ui.team_publish_tableWidget.rowCount() == 0:
        dial = QtWidgets.QMessageBox()
        dial.setWindowTitle("Error")
    ...
    if not self.ui.team_publish_tableWidget.selectedItems():
        dial = QtWidgets.QMessageBox()
        dial.setWindowTitle("Error")
    ...
    tableRow = self.ui.team_publish_tableWidget.currentRow()
    self.ui.team_publish_tableWidget.removeRow(tableRow)